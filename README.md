...
> JimuReport 平台 Claude Code 技能集合，通过自然语言驱动，让 AI 自动生成 **AI 报表**、**AI 大屏** 和 **AI 仪表盘**，覆盖数据可视化全场景。一句话描述需求即可生成专业的报表与可视化页面，告别繁琐的拖拽设计。
>
> - 技能官网： https://jimureport.com/skills
> - AI 报表提示词： https://help.jimureport.com/rumen/jimureport-prompts
> - AI 大屏提示词： https://help.jimureport.com/rumen/bigscreen-prompts

#### 1. jimureport — AI 报表生成器

**一句话**：用自然语言描述报表需求或提供截图，AI 自动生成 Excel 风格可视化报表并通过 API 创建。

**核心能力**：
- AI 智能识别：根据自然语言或截图自动生成报表布局与数据绑定
- 可视化 Excel 设计器风格，支持自由布局、合并单元格、多 Sheet
- 数据绑定：`#{数据集编码.字段名}` 模板语法
- 支持数据填报（submitForm）、参数传递、动态过滤
- 精细打印控制：纸张大小、边距、方向，支持套打/分页打印
- CSS/JS/Python 增强能力
- 多种数据源支持：SQL 数据集、API 数据集、JSON 数据集、文件数据集
...
| 3 | 横向分组小计 | 动态列分组与聚合 |
| 4 | 多源报表 | 关联多个数据源 |
| 5 | 文件数据集报表 | 基于 Excel/CSV |
| 6 | 主子报表 + 循环块 | 分页显示关联数据 |
| 7 | 查询条件报表 | 字段级查询过滤 |
| 8 | 二维码/条形码报表 | 资产、票据追踪 |
| 9 | 钻取报表 | 表格/图表 → 报表/网页跳转 |
| 10 | 联动报表 | 多组件交互式刷新 |
| 11 | 图表展示 | 集成 28+ 种 ECharts 图表 |

> 与 Online 报表（cgreport）互补：积木报表侧重复杂布局和打印，Online 报表侧重快速配置。

---

#### 2. jimubi-bigscreen — AI 大屏生成器

**一句话**：用自然语言描述大屏需求，AI 自动生成全屏数据可视化大屏并通过 API 创建。

**核心能力**：
- AI 自动布局：根据业务主题智能选择组件、配色与排版
- 全屏展示模式，像素绝对定位（x/y/w/h），无栅格约束
- 默认 1920×1080 深色主题、星空背景，适用于监控室/展厅/展示墙
- 平台命名色板：复古、商务、科技等多种气质配色
- 12 套装饰条 + 13 套边框预设增强视觉效果
- 丰富组件：数字翻牌、折线图、柱状图、饼图、地图、滚动表格、排行榜、3D 柱状图、彩色仪表盘等

**推荐两步工作流**：
1. **第一步**：使用纯静态 mock 数据快速出视觉效果，验证组件搭配、布局与配色
2. **第二步**：按需逐个组件替换为真实数据源（API/JSON/SQL），每次改 1-2 个便于排错

**高级交互能力**：
- 查询表单驱动整屏数据按条件刷新
- 多屏切换：导航组件切换不同页面
- 多级地图钻取：省 → 市 → 区逐级下钻
- 图表间联动：点击一个图表刷新多个目标组件

**18 大行业场景**（提示词示例库）：物流港口、酒店集团、在线教育、政务服务、餐饮连锁、智慧农业、电竞赛事、银行网点等。

**组件编码直指定**（高级用法）：`JBar3d`（3D 柱状图）、`JColorGauge`（彩色仪表盘）、`JGaoDeMap`（高德地图）、`JFlashList`（闪烁告警列表）等。

> 注意：大屏与仪表盘使用完全不同的布局和样式体系，仪表盘请使用 `jimubi-dashboard`。

---

#### 3. jimubi-dashboard — AI 仪表盘生成器

**一句话**：用自然语言描述看板需求，AI 自动生成栅格布局数据仪表盘并通过 API 创建。

**核心能力**：
- AI 智能栅格规划：根据指标数量与类型自动分配卡片宽度
- 24 列栅格布局，亮色主题，卡片式设计
- 适用于日常数据看板、运营统计面板、移动端看板
- 支持数字卡片、折线图、柱状图、饼图、表格、排行榜、仪表盘、地图等组件
- 智能栅格分配：数字卡片 4 个一行，图表半宽或全宽自动排列
- 卡片头与 ECharts 标题智能去重
- 支持联动、查询、钻取、跳转、自定义 JS 等交互配置
...
```text
# AI 报表
> 帮我做一个按部门分组的员工工资统计报表，包含纵向分组、小计、合计

# AI 大屏
> 做一个智慧物流监控大屏，包含全国货运地图、实时订单流水、TOP10 仓储排行

# AI 仪表盘
> 生成一个销售运营看板：4 个 KPI 卡片（销售额/订单量/转化率/客单价）+ 月度趋势折线图 + 区域占比饼图
```
